cso <- read.csv("Sordidus.csv",stringsAsFactors = FALSE)
...
clrs <- c("#FF6F00B2","#C71000B2","#008EA0B2","#8A4198B2","#5A9599B2")
...
overlap.plot(over.stat, col = clrs, mean.cred.col = "#3D3B25B2", equal.axis = TRUE, 
 ...
 tiff("cso.overlap.tiff", width = 9, height = 6.5, units = 'in', res = 300)
 overlap.plot(over.stat, col = clrs, mean.cred.col = "#3D3B25B2", equal.axis = TRUE,
              xlab = "Overlap probability (%) -- Niche region size: 95%")
dev.off()
...
cso.niche.size <- boxplot(fish.size, col = clrs, pch = 16, cex = .5,
                         ylab = "Niche Size", xlab = "Location", ylim = c(0,30))

tiff("cso.niche.size.tiff", width = 9, height = 6.5, units = 'in', res = 300)
